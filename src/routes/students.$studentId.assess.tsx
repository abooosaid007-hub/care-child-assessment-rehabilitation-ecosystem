...
export const Route = createFileRoute("/students/$studentId/assess")({
  head: () => ({ meta: [{ title: "Assessment Intake — CARE" }] }),
...
function AssessmentPage() {
  ...
  // Initial load: fetch student + draft assessment
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: s, error: sErr } = await supabase
        .from("students")
        .select(
          "id, student_code, first_name, date_of_birth, gender, primary_condition, comorbid_conditions, under_observation, severity, complexity_flag",
        )
        .eq("id", studentId)
        .maybeSingle();
      if (cancelled) return;
      if (sErr) {
        setLoadError(`Failed to load student: ${sErr.message}`);
        setLoading(false);
        return;
      }
      if (!s) {
  ...
  }, [studentId, authLoading, user, profile]);

  // Pre-fill consultant name once profile is available
  useEffect(() => {
          ...
          <div className="flex items-center justify-between gap-3 mb-2">
            <Button variant="ghost" size="sm" asChild>
              <Link to="/students/$studentId" params={{ studentId }}>
                <ArrowLeft className="h-4 w-4" /> {student.first_name}
              </Link>
            </Button>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <Button
            variant="outline"
            disabled={section === 9}
            onClick={() => { void save(true); setSection((s) => Math.min(9, s + 1)); }}
          >
            Next
          </Button>
        </div>
